    ...
    def __init__(self, in_c, args=None):
        ...
        if ham_type == "NMF":
            self.lower_bread = nn.Sequential(
                nn.Conv2d(in_c, D, 1), nn.ReLU(inplace=True)
            )
        else:
        ...
        self.upper_bread = nn.Sequential(
            nn.Conv2d(D, in_c, 1, bias=False), norm_layer(in_c)
        )
...
class HamburgerV2(nn.Module):
    def __init__(self, in_c, args=None):
        ...
        if ham_type == "NMF":
            self.lower_bread = nn.Sequential(
                nn.Conv2d(in_c, C, 1), nn.ReLU(inplace=True)
            )
        else:
...
def get_hamburger(version):
    burgers = {"V1": HamburgerV1, "V2": HamburgerV2, "V2+": HamburgerV2Plus}

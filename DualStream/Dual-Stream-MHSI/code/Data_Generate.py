...
class Data_Generate_Bile(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img_path.endswith(".hdr"):
            ...
            img = np.load(img_path)  # our image

        mask_path = self.seg_paths[index]
        if mask_path.endswith(".npy"):
            mask = np.load(mask_path)  # our mask
        ...
        if self.hyperhsi:
            ...
            for i in range(img.shape[-1]):
                Ws[i] = normalized_mutual_info_score(
                    img_pca.reshape(-1), img[:, :, i].reshape(-1)
                )
...
class Data_Generate_Transc(Dataset):
    def __init__(self, gene_exp_path, label_paths):
        self.gene_exp_path = gene_exp_path  # gene expression
        self.label_paths = label_paths  # labels

    def __getitem__(self, index):
        gene_path = self.gene_exp_path[index]
        gene = np.load(gene_path)  # our image

        label_path = self.label_paths[index]
        label = np.load(label_path)  # our mask

        gene = gene[:32, :32].astype(np.float32)
        label = label[:32, :32].astype(np.float32)

        gene = gene.transpose(2, 0, 1)

        return gene, label
    
    def __len__(self):
        return len(self.gene_exp_path)

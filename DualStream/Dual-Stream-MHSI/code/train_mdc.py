...
from local_utils.metrics import (
    iou,
    dice,
    sensitivity,
    specificity,
    hausdorff_distance_case,
    eval_f1score,
    multi_iou,
)
...
def main(args):
    ...
    if dataset == "brain":
        train_images_path = [
            os.path.join(root_path, i, i, "raw.hdr") for i in train_files
        ]
        train_masks_path = [
            os.path.join(root_path, i, i, "gtMap.hdr") for i in train_files
        ]
        ...
        test_images_path = [
            os.path.join(root_path, i, i, "raw.hdr") for i in test_files
        ]
        test_masks_path = [
            os.path.join(root_path, i, i, "gtMap.hdr") for i in test_files
        ]
    elif dataset == "dental":
        train_images_path = [os.path.join(root_path, i + ".tif") for i in train_files]
        train_masks_path = [
            os.path.join(root_path, i + "_masks.tif") for i in train_files
        ]
        ...
        test_masks_path = [
            os.path.join(root_path, i + "_masks.tif") for i in test_files
        ]
    elif dataset == "bio":
        train_images_path = [
            os.path.join(root_path, f"gene_expre_matrix_{i}.npy") for i in train_files
        ]
        train_masks_path = [
            os.path.join(root_path, f"cell_type_matrix{i}") for i in train_files
        ]
        val_images_path = [
            os.path.join(root_path, f"gene_expre_matrix_{i}.npy") for i in val_files
        ]
        val_masks_path = [
            os.path.join(root_path, f"cell_type_matrix{i}") for i in val_files
        ]
        test_images_path = [
            os.path.join(root_path, f"gene_expre_matrix_{i}.npy") for i in test_files
        ]
        test_masks_path = [
            os.path.join(root_path, f"cell_type_matrix{i}") for i in test_files
        ]
    else:
        train_images_path = [os.path.join(images_root_path, i) for i in train_files]
        train_masks_path = [
            os.path.join(mask_root_path, f"{i[:-4]}.{mask_extension}")
    ...
    train_db = Data_Generate_Bile(
        train_images_path,
        train_masks_path,
        transform=transform,
        principal_bands_num=principal_bands_num,
        cutting=cutting,
        envi_type=envi_type,
        multi_class=classes,
    )
    ...
    if local_rank == 0:
        ...
        save_dict(
            os.path.join(f"{output_path}/{experiment_name}", "args.csv"), args.__dict__
        )

    if net_type == "backbone":
        print(
            f"choose backbone is {backbone} and spatial_pretrain is {spatial_pretrain}"
        )
    ...
    if scheduler_type == "cos":
        ...
        scheduler = CosineLRScheduler(
            optimizer, t_initial=args.epochs, warmup_t=9, warmup_prefix=True
        )
    else:
    ...
    def sigint_handler(signal, frame):
        ...
        history_pd.to_csv(
            os.path.join(f"{output_path}/{experiment_name}", "log.csv"), index=False
        )

        cm_pd = pd.concat(confusion_matrix_history, keys=range(1, epoch + 2))
        cm_pd.to_csv(
            os.path.join(f"{output_path}/{experiment_name}", f"confusion_matrix.csv"),
            index_label=["Epoch"],
            index=True,
        )
    ...
    for epoch in range(epochs):
        ...
        if classes == 1:
            ...
            val_dice = np.array(
                [
                    dice(np.where(l == b, 1, 0), np.where(o == b, 1, 0))
        ...
        print(
            "epoch {}/{}\t LR:{}\t train loss:{}\t val_dice:{} \t val_iou:{}".format(
                epoch + 1,
                epochs,
        ...
        confusion_matrix_history.append(
            pd.DataFrame(cm, index=range(classes), columns=range(classes))
        )
        ...
        if best_val <= reduce_tensor(val_dice):
            ...
            torch.save(
                ...
                os.path.join(
                    f"{args.output}/{args.experiment_name}",
            ...
            best_outs = outs
            best_labels = labels
            confusion_matrix_history[-1].to_csv(
                os.path.join(
                    f"{output_path}/{experiment_name}", f"best_confusion_matrix.csv"
                )
            )
        ...
        if local_rank == 0:
            history_pd = pd.DataFrame(history)
            history_pd.to_csv(
                os.path.join(f"{output_path}/{experiment_name}", f"log.csv"),
                index=False,
            )
            ...
            cm_pd.to_csv(
                os.path.join(
                    f"{output_path}/{experiment_name}", f"confusion_matrix.csv"
                ),
                index_label=["Epoch"],
                index=True,
            )
    ...
    ConfusionMatrixDisplay.from_predictions(
        best_labels.flatten(),
        best_outs.flatten(),
        display_labels=np.unique(best_labels),
        include_values=remove_cm_values,
    )
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_divide", "-dd", type=str, default="train_val_test.json"
    )
    ...
    parser.add_argument(
        "--link_position", "-link_p", type=int, default=[0, 0, 0, 1, 0, 1], nargs="+"
    )
    parser.add_argument(
        "--loss_function", "-lf", default="dicebce", choices=["dicebce", "bce"]
    )
    parser.add_argument(
        "--spe_kernel_size", "-sks", type=int, default=1
    )  # , nargs='+')
    ...
    parser.add_argument(
        "--merge_spe_downsample", "-msd", type=int, default=[2, 1], nargs="+"
    )
    parser.add_argument("--scheduler", "-sc", default="cos", choices=["cos", "warmup"])
    parser.add_argument(
        "--net", "-n", default="dual", type=str, choices=["backbone", "dual"]
    )
    ...
    parser.add_argument(
        "--remove_cm_values", "-rcm", action="store_false", default=True
    )

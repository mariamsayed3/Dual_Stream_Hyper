...
class SMD(nn.Module):
    def __init__(
        ...
        self.depthwiseconv = nn.Conv2d(
            ...
            kernel_size=spe_reduction,
            stride=spe_reduction,
            groups=in_channels,
        )
...
class merge_block(nn.Module):
    def __init__(
        self, spa_in_channels, spe_in_channels, bands_group=4, merge_spe_downsample=2
    ):
        ...
        self.spa_in_channels = spa_in_channels
        self.spe_in_channels = spe_in_channels

        self.merge_conv_spa = nn.Sequential(
            nn.Conv2d(
                spa_in_channels + spe_in_channels,
                spa_in_channels,
                kernel_size=3,
                stride=1,
                padding=1,
    ...
    def forward(self, spa_input, spe_input):  # (b g) c h w , b c s h w
        # print(
        #     "here: ",
        #     self.spa_in_channels,
        #     self.spe_in_channels,
        #     self.bands_group,
        #     self.merge_spe_downsample,
        # )
        spa_input = rearrange(spa_input, "(b g) c h w -> b g c h w", g=self.bands_group)
        spe2spa_input = reduce(
            spe_input,
            "b c (s1 p) h w -> b s1 c h w",
            "mean",
            p=self.merge_spe_downsample,
        )
        spa_input = self.merge_conv_spa(
            rearrange(
                torch.cat((spa_input, spe2spa_input), 2), "b g c h w -> (b g) c h w"
            )
        )
...
class SpatialSpetralMixStream(nn.Module):
    def __init__(
        ...
        self.spe_feature_dim = [
            1,
            spectral_hidden_feature * 1,
            spectral_hidden_feature * 2,
            spectral_hidden_feature * 4,
            spectral_hidden_feature * 8,
            spectral_hidden_feature * 16,
        ]
        ...
        self.spatial_backbone = get_encoder(
            name=backbone,
            in_channels=spectral_channels // bands_group,
            depth=5,
            weights="imagenet" if spatial_pretrain else None,
        ...
        self.merge_stages = nn.ModuleList(
            [
                merge_block(
                    self.spa_feature_dim[i - 1],
                    self.spe_feature_dim[idx + 1],
        ...
        self.spe_encoder_stages = nn.ModuleList(
            [
                SMD(
                    self.spe_feature_dim[idx],
                    self.spe_feature_dim[idx + 1],
                    spa_reduction[idx],
        ...
        if decode_choice != "deeplabv3plus":
            self.spaspe_attention_head = LD(
                in_channel=self.spa_feature_dim[-1],
                height=hw[0] // (2 ** len(self.spa_feature_dim)),
                width=hw[1] // (2 ** len(self.spa_feature_dim)),
                rank=rank,
            )
        else:
            self.spaspe_attention_head = LD(
                ...
                width=hw[1] // (2 ** (len(self.spa_feature_dim) - 1)),
                rank=rank,
            )
    ...
    def forward(self, input):
        ...
        spa_input = rearrange(
            spa_input, "b (g c1) h w -> (b g) c1 h w", g=self.bands_group
        )
...
class SST_Seg_Dual(nn.Module):
    def __init__(
        ...
        self.encoder = SpatialSpetralMixStream(
        ...
        if decode_choice == "unet":
            self.decoder = decoders.unet.decoder.UnetDecoder(
                encoder_channels=list(
                    [spatial_channels] + self.encoder.spa_feature_dim
                ),

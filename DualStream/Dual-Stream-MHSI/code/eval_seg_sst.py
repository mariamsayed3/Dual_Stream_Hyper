...
def main(args):
    ...
    test_masks_path = [
        os.path.join(mask_root_path, f"{i[:-4]}.png") for i in test_files
    ]
    ...
    test_loader = DataLoader(
        test_db, batch_size=batch, shuffle=False, num_workers=worker, drop_last=False
    )
    ...
    model.load_state_dict(
        torch.load(args.pretrained_model, map_location="cuda:0"), strict=True
    )
    ...
    history["params_M"] = 0.0  # params
    history["MACs_G"] = 0.0  # macs
    ...
    if args.vis:
        os.makedirs(
            f"{'/'.join(args.pretrained_model.split('/')[:-1])}/vis/", exist_ok=True
        )
        dices = np.array([dice(l, o) for l, o in zip(labels, outs)])
        _ = [
            cv2.imwrite(
                f"{'/'.join(args.pretrained_model.split('/')[:-1])}/vis/{test_files[i]}_{dices[i]}.jpg",
                (outs[i][0] * 255).astype(np.uint8),
            )
            for i in range(len(outs))
        ]
        _ = [
            cv2.imwrite(
                f"{'/'.join(args.pretrained_model.split('/')[:-1])}/vis/{test_files[i]}.jpg",
                (labels[i][0] * 255).astype(np.uint8),
            )
    ...
    history_pd.to_csv(
        os.path.join(
            f"{'/'.join(args.pretrained_model.split('/')[:-1])}", f"log_eval.csv"
        ),
        index=False,
    )
